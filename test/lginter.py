...
def load_excel(file_path, output_path):
    try:
        xl_data= pd.read_excel(file_path, engine= 'openpyxl')
        xl_data.to_csv(output_path, index= False)
        loader= CSVLoader(file_path=output_path, encoding="utf-8", csv_args={
                'delimiter': ','})
        data= loader.load()
        return data
...
def main_chain(vectorstore):
    ...
    custom_prompt = PromptTemplate(
        template=(
            ...
            "{{\"system_output\": \"<ANALYSIS>\", \"user_input\": \"{input_text}\", \"extractor\": \"<PROMPT>\"}}\n"
            "Where:\n"
            "- \"system_output\" is the analysis of {input_text}, with grammatical mistakes fixed, look for the intention in the message."
            "- \"user_input\" contains the corrected version of {input_text} with grammatical mistakes fixed.\n"
            "- \"extractor\" is replaced with the appropriate prompt based on the context of the grammatically correct {input_text} as follows:\n\n"
            "If the {input_text} contains user information (e.g., name or email):\n"
            "Replace <PROMPT> with:\n"
            "  'Respond just in a JSON format, nothing more: {{\"name\": \"<NAME>\", \"email\": \"<EMAIL>\"}}, use double quotes always for each element inside of the JSON.'\n\n"
            ...
            "Respond must be only in the following format, no extra words, use double quotes always for the elements of the JSON as it shows the following format: "
            "[{{\"product_id\": \"<string>\", \"product_name\": \"<string>\", \"quantity\": <integer>, \"value\": <float>}}]. '\n\n"
            "Important:\n"
...
def extraction_chain(csv_data):

    main_llm = ChatGroq(
        model="llama-3.1-70b-versatile",
        temperature=0
    )
    retriever = csv_data.as_retriever()
...
# Main RAG system
def agent_system(main_vectorstore, csv_verctorstore):
    main_chain_instance= main_chain(main_vectorstore)
    extraction_chain_instance= extraction_chain(csv_verctorstore)
...
def interaction_system(user_input, main_chain, extraction_chain):
    ...
    raw_response= main_chain.invoke({
        "user_input": user_input
        })
    #raw_response= main_chain.invoke({"input_text": user_input}).content
    print(raw_response)


    #raw_extraction_response= extraction_chain.invoke({
    #    "question": str(raw_response['extractor']),
    #    "chat_history": []
    #})

    #print(raw_extraction_response)
    #extraction_response= json.loads(raw_extraction_response["answer"])
...
extraction_vectorstore= csv_vectorstore(csv_document)
...
if __name__ == "__main__":
    ...
    main_chain, extraction_chain= agent_system(main_vectorstore, extraction_vectorstore)
